...
const CustomAlert = ({ message, type = 'error', onClose }) => {
  ...
  return (
    <div className={`fixed top-4 right-4 z-[100] px-6 py-4 rounded-xl border flex items-start gap-3 shadow-2xl backdrop-blur-md max-w-md animate-in slide-in-from-top-4 ${bgClass}`}>
      <AlertCircle size={20} className="shrink-0 mt-0.5" />
      <div className="flex flex-col gap-1">
        <span className="text-xs font-bold uppercase tracking-wider">Pemberitahuan Sistem</span>
        <span className="text-xs leading-relaxed font-medium">{message}</span>
      </div>
      <button onClick={onClose} className="ml-auto hover:opacity-70 shrink-0"><X size={16} /></button>
    </div>
  );
};
...
const App = () => {
  ...
  const [config, setConfig] = useState({
    style: 'COMMERCIAL', composition: 'Balanced', length: 4, aspectRatio: '9:16', environment: 'Mirror Selfie di Kamar (Cermin Besar)', customEnvironment: ''
  });
  ...
  const detectProductWithAI = async (file, newUploads) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setAppError(getFriendlyErrorMessage(data.error.message));
        return;
      }
  ...
  const handleGenerateContent = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (promptData.error) {
          throw new Error(getFriendlyErrorMessage(promptData.error.message));
        }
              ...
              {showTextInput && (
                <div className={`bg-slate-800/80 p-6 rounded-2xl border border-slate-700 mt-4 ${selectedMode !== 'model-product' ? 'max-w-2xl mx-auto' : ''}`}>
                  ...
                  <div className="mt-5 flex justify-end">
                    <button onClick={handleProceedToStyle} disabled={!isReadyForStep3 || isDetecting} className="px-8 py-3 bg-yellow-500 text-[#0f172a] font-bold rounded-xl disabled:opacity-50 transition-colors">Lanjut ke Visual Box</button>
                  </div>
                </div>
              )}
            </section>
